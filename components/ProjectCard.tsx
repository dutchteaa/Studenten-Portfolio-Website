  ...
  return (
    <div className="card card-lift overflow-hidden">
      {afbeeldingUrl ? (
        ...
        <div className="w-full h-44 flex items-center justify-center" style={{ background: 'var(--gradient-subtle)' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: 'var(--accent-3)' }}>
      ...
      <div className="p-5">
        ...
        {studentNaam && (
          <p className="text-xs mt-0.5" style={{ color: 'var(--accent-3)' }}>door {studentNaam}</p>
        )}
        <p className="text-sm mt-2.5 line-clamp-3 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{beschrijving}</p>
        {(githubLink || demoLink) && (
          <div className="flex gap-2 mt-4 pt-3.5" style={{ borderTop: '1px solid var(--border)' }}>
            {githubLink && (
